...
class Database:
    ...
    async def connect(self) -> None:
        ...
        for migration in sorted(Path(__file__).with_name("migrations").glob("*.sql")):
            ...
            applied = await (await self.connection.execute(
        ...
        self._reader = await aiosqlite.connect(self.path)
    ...
    async def bars_between(self, symbol: str, bar_size: str, start: str, end: str):
        rows = await (await self.read_connection.execute(
            """SELECT * FROM market_bars WHERE symbol=? AND bar_size=?
               AND timestamp>=? AND timestamp<=? AND use_rth=1
               ORDER BY timestamp DESC""", (symbol, bar_size, start, end)
        )).fetchall()
    ...
    async def ping(self) -> bool:
        row = await (await self.read_connection.execute("SELECT 1 AS ok")).fetchone()
        return bool(row and row["ok"] == 1)

    async def list_stocks(self) -> list[dict[str, Any]]:
        rows = await (
            await self.read_connection.execute(
    ...
    async def save_report(self, report: ReportCreate) -> dict[str, Any]:
        async with self.transaction():
            await self.write_report(report)
        stored = await self.get_report(report.id)
        assert stored is not None
        return stored

    async def write_report(self, report: ReportCreate) -> None:
        """Write inside the caller's transaction, allowing atomic job/outbox commits."""
        now = utc_now()
        await self.connection.execute(
            """
            INSERT INTO reports(
